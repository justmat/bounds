...
function lfo.init()
  for i = 1, number_of_outputs do
    ...
    -- lfo depth
    params:add_number(i .. "lfo_depth", "lfo depth", 0, 100, 100)
    params:set_action(i .. "lfo_depth", function(value) lfo[i].depth = value end)
    -- lfo offset
    params:add_control(i .."offset", "offset", controlspec.new(-4.0, 3.0, "lin", 0.1, 0.0, ""))
    params:set_action(i .. "offset", function(value) lfo[i].offset = value end)
    -- lfo speed
    params:add_control(i .. "lfo_freq", "lfo freq", controlspec.new(0.01, 10.0, "lin", 0.1, 0.01, ""))
